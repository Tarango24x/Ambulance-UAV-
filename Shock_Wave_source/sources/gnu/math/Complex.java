...
public abstract class Complex extends Quantity {
    ...
    public boolean isExact() {
        return mo10972re().isExact() && mo10971im().isExact();
    }

    /* Debug info: failed to restart local var, previous not found, register: 4 */
    public Complex toExact() {
        RealNum re = mo10972re();
        RealNum im = mo10971im();
    ...
    public Complex toInexact() {
        return isExact() ? this : new DComplex(mo10972re().doubleValue(), mo10971im().doubleValue());
    }
    ...
    public double doubleValue() {
        return mo10972re().doubleValue();
    }

    public double doubleImagValue() {
        return mo10971im().doubleValue();
    }
    ...
    public static boolean equals(Complex x, Complex y) {
        return x.mo10972re().equals(y.mo10972re()) && x.mo10971im().equals(x.mo10971im());
    }
    ...
    public static int compare(Complex x, Complex y) {
        int code = x.mo10971im().compare(y.mo10971im());
        return code != 0 ? code : x.mo10972re().compare(y.mo10972re());
    }
    ...
    public boolean isZero() {
        return mo10972re().isZero() && mo10971im().isZero();
    }

    public String toString(int radix) {
        if (mo10971im().isZero()) {
            return mo10972re().toString(radix);
        }
        String imString = mo10971im().toString(radix) + "i";
        if (imString.charAt(0) != '-') {
            imString = "+" + imString;
        }
        return !mo10972re().isZero() ? mo10972re().toString(radix) + imString : imString;
    }

    public static Complex neg(Complex x) {
        return make(x.mo10972re().rneg(), x.mo10971im().rneg());
    }
    ...
    public static Complex add(Complex x, Complex y, int k) {
        return make(RealNum.add(x.mo10972re(), y.mo10972re(), k), RealNum.add(x.mo10971im(), y.mo10971im(), k));
    }
    ...
    public static Complex times(Complex x, Complex y) {
        RealNum x_re = x.mo10972re();
        RealNum x_im = x.mo10971im();
        RealNum y_re = y.mo10972re();
        RealNum y_im = y.mo10971im();
