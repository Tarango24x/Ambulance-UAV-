...
public class Navigation extends AndroidNonvisibleComponent implements Component {
    ...
    enum TransportMethod {
        ...
        private TransportMethod(String method2) {
            this.method = method2;
        }
    ...
    public void RequestDirections() {
        if (this.apiKey.equals("")) {
            this.form.dispatchErrorOccurredEvent(this, AUTH.WWW_AUTH_RESP, ErrorMessages.ERROR_INVALID_API_KEY, new Object[0]);
            return;
        }
    ...
    public void performRequest(GeoPoint start, GeoPoint end, TransportMethod method2) throws IOException, JSONException {
        ...
        connection.setRequestProperty(HTTP.CONTENT_TYPE, "application/json; charset=UTF-8");
        connection.setRequestMethod(HttpPost.METHOD_NAME);
        connection.setRequestProperty(AUTH.WWW_AUTH_RESP, this.apiKey);
        try {
            byte[] postData = ("{\"coordinates\": " + JsonUtil.getJsonRepresentation(getCoordinates(start, end)) + ", \"language\": \"" + this.language + "\"}").getBytes(HTTP.UTF_8);
