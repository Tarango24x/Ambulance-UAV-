...
public class HoneycombWebViewClient extends FroyoWebViewClient<WebViewer> {
    ...
    public WebResourceResponse shouldInterceptRequest(WebView view, WebResourceRequest request) {
        ...
        if (request.getUrl().getAuthority().equals("localhost") || request.getUrl().toString().startsWith(ASSET_PREFIX)) {
    ...
    public WebResourceResponse handleAppRequest(String url) {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= 21) {
                return new WebResourceResponse("text/plain", "utf-8", 404, "Not Found", (Map) null, error);
            }
