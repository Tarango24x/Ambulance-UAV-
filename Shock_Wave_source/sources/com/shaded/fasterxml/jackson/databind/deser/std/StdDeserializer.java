        ...
        if (currentToken == JsonToken.VALUE_STRING) {
            ...
            try {
                ...
                if (length > 9) {
                    ...
                    throw deserializationContext.weirdStringException(trim, this._valueClass, "Overflow: numeric value (" + trim + ") out of range of int (" + Integer.MIN_VALUE + " - " + ActivityChooserView.ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED + ")");
                } else if (length != 0) {
        ...
        if (currentToken == JsonToken.VALUE_STRING) {
            ...
            try {
                ...
                if (length > 9) {
                    ...
                    throw deserializationContext.weirdStringException(trim, this._valueClass, "Overflow: numeric value (" + trim + ") out of range of Integer (" + Integer.MIN_VALUE + " - " + ActivityChooserView.ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED + ")");
                } else if (length == 0) {
    ...
    public Date _parseDate(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException, JsonProcessingException {
        ...
        if (currentToken == JsonToken.VALUE_STRING) {
            try {
                ...
                throw deserializationContext.weirdStringException((String) null, this._valueClass, "not a valid representation (error: " + e.getMessage() + ")");
            }
        } else {
            throw deserializationContext.mappingException(this._valueClass, currentToken);
        }
