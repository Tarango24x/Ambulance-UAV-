...
public class JsonWebToken {
    ...
    public static JsonWebToken decode(String token) throws IOException {
        ...
        C06861 r3 = new TypeReference<Map<String, Object>>() {
        };
        Map<String, Object> claims2 = (Map) JsonHelpers.getMapper().readValue(Base64.decode(fixLength(parts[1])), (TypeReference) r3);
        String signature2 = parts[2];
        Object data2 = claims2.get("d");
        claims2.remove("d");
        return new JsonWebToken((Map) JsonHelpers.getMapper().readValue(Base64.decode(fixLength(parts[0])), (TypeReference) r3), claims2, data2, signature2);
    }
}
