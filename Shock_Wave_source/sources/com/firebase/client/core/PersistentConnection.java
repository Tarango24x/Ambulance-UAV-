...
public class PersistentConnection implements Connection.Delegate {
    ...
    /* access modifiers changed from: package-private */
    public void onDisconnectPut(Path path, Object data, Firebase.CompletionListener onComplete) {
        if (canSendWrites()) {
    ...
    /* access modifiers changed from: package-private */
    public void onDisconnectMerge(Path path, Map<String, Object> updates, Firebase.CompletionListener onComplete) {
        if (canSendWrites()) {
        ...
        if (connected()) {
            sendAction(REQUEST_ACTION_UNAUTH, new HashMap(), new ResponseListener() {
                public void onResponse(Map<String, Object> response) {
                    ...
                    listener.onComplete(error, (Firebase) null);
                }
            });
        }
    }
    ...
    private void sendOnDisconnect(String action, Path path, Object data, final Firebase.CompletionListener onComplete) {
        ...
        sendAction(action, request, new ResponseListener() {
            public void onResponse(Map<String, Object> response) {
                ...
                if (onComplete != null) {
                    onComplete.onComplete(error, (Firebase) null);
                }
            }
        });
    }
    ...
    public OutstandingListen removeListen(QuerySpec query) {
        ...
        OutstandingListen outstandingListen = this.listens.get(query);
    ...
    private void onDataPush(String action, Map<String, Object> body) {
        ...
        if (action.equals("d") || action.equals("m")) {
            ...
            for (Map<String, Object> range : (List) payloadData2) {
                ...
                rangeMerges.add(new RangeMerge(startString != null ? new Path(startString) : null, endString != null ? new Path(endString) : null, NodeUtilities.NodeFromJSON(range.get("m"))));
            }
            if (!rangeMerges.isEmpty()) {
                this.delegate.onRangeMergeUpdate(new Path(pathString2), rangeMerges, tag2);
            } else if (this.logger.logsDebug()) {
                this.logger.debug("Ignoring empty range merge for path " + pathString2);
            }
        } else if (action.equals("c")) {
            onListenRevoked(new Path((String) body.get("p")));
    ...
    private void sendAuthHelper(final boolean restoreWritesAfterComplete) {
        if (!$assertionsDisabled && !connected()) {
            ...
            sendAction(REQUEST_ACTION_AUTH, request, new ResponseListener() {
                public void onResponse(Map<String, Object> response) {
                    ConnectionState unused = PersistentConnection.this.connectionState = ConnectionState.Connected;
                    if (credential == PersistentConnection.this.authCredential) {
                        ...
                        if (status.equals("ok")) {
                            ...
                            AuthCredential unused2 = PersistentConnection.this.authCredential = null;
                    ...
                    if (((OutstandingPut) PersistentConnection.this.outstandingPuts.get(Long.valueOf(j))) == put) {
                        ...
                        if (onComplete != null) {
                            ...
                            if (status.equals("ok")) {
                                ...
                                onComplete.onComplete(FirebaseError.fromStatus(status, (String) response.get("d")), (Firebase) null);
                            }
                        }
    ...
    private void sendListen(final OutstandingListen listen) {
        ...
        sendAction("q", request, new ResponseListener() {
            public void onResponse(Map<String, Object> response) {
                ...
                if (!status.equals("ok")) {
                    OutstandingListen unused = PersistentConnection.this.removeListen(listen.getQuery());
